...
class igrfclass:
    ...
    def __init__(self, coeff_file=None):
        ...
        self.update_coefficients(coeff_file)

    def update_coefficients(self,coeff_file=None):
        """
        coeff_file : File path to a different set of coefficients.

        This method updates the coefficients to be used in IGRF.
        """
        if not type(coeff_file) == type(None):
            self.coeff_file = coeff_file
        self._read_coeff_file(self.coeff_file)
        self._get_m_n_schmidt(self.max_n)
